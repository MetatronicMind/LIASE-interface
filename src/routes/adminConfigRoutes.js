...
const requireAdmin = (req, res, next) => {
  ...
  if (req.method === "GET") {
    if (
      !req.user.permissions?.admin_config?.read &&
      !req.user.permissions?.admin_config?.manage &&
      !req.user.permissions?.admin_config?.write
    ) {
      return res
        .status(403)
        .json({ error: "Config read permissions required" });
    }
    return next();
  }
...
const requireSuperAdmin = (req, res, next) => {
  ...
  if (role !== "superadmin" && role !== "admin") {
    console.warn(
      `[AdminConfig] Access denied. Required: superadmin/admin, Actual: ${req.user.role}`,
    );
...
router.get(
  ...
  async (req, res) => {
    try {
      console.log(
        "Fetching system config for user:",
        req.user.id,
        "Org:",
      ...
      const systemConfig = {
        aiInferenceEndpoints: {
          primary:
            savedData.aiInferenceEndpoints?.primary ||
            process.env.AI_INFERENCE_URL_1 ||
            "http://48.217.12.7/get_AI_inference",
          secondary:
            savedData.aiInferenceEndpoints?.secondary ||
            process.env.AI_INFERENCE_URL_2 ||
            "http://4.157.127.230/get_AI_inference",
          tertiary:
            savedData.aiInferenceEndpoints?.tertiary ||
            process.env.AI_INFERENCE_URL_3 ||
            "http://4.157.29.153/get_AI_inference",
          quaternary:
            savedData.aiInferenceEndpoints?.quaternary ||
            process.env.AI_INFERENCE_URL_4 ||
            "http://4.236.195.153/get_AI_inference",
        },
        r3XmlEndpoint:
          savedData.r3XmlEndpoint ||
          process.env.R3_XML_ENDPOINT ||
          "http://4.236.195.153/get_r3_xml",
        // Make sure to use the saved PubMed endpoint if available, otherwise default
        pubmedApiEndpoint:
          savedData.pubmedApiEndpoint ||
          process.env.PUBMED_API_ENDPOINT ||
          "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/",
        pmidListEndpoint:
          savedData.pmidListEndpoint ||
          process.env.PMID_LIST_ENDPOINT ||
        ...
        aiProcessing: {
          ...
          circuitBreakerThreshold:
            parseInt(process.env.AI_CIRCUIT_BREAKER_THRESHOLD) || 5,
        ...
        scheduler: {
          ...
          notificationProcessingInterval:
            parseInt(process.env.NOTIFICATION_INTERVAL) || 10,
          ...savedData.scheduler,
        },
        security: {
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message:
          "System configuration saved successfully. Note: Environment variable changes require server restart.",
        config,
      });
    } catch (error) {
...
router.post(
  ...
  // authenticateToken is already applied in app.js
  requireSuperAdmin,
  [body("type").notEmpty(), body("url").isURL()],
  validate,
  async (req, res) => {
    try {
...
router.post(
  ...
  authenticateToken,
  requireAdmin,
  [body("assetType").isIn(["logo", "favicon"]), body("dataUrl").notEmpty()],
  validate,
  async (req, res) => {
    try {
