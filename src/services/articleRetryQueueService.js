...
class ArticleRetryQueueService {
  constructor() {
    ...
    console.log(
      "[ArticleRetryQueue] Service initialized with failsafe retry system",
    );
  }
  ...
  startBackgroundProcessor() {
    setInterval(async () => {
      try {
        ...
        console.error(
          "[ArticleRetryQueue] Background processor error:",
          error.message,
        );
      }
   ...
   */
  async processArticlesWithGuarantee(
    articles,
    searchParams,
    organizationId,
    userId,
    ...
    console.log(`\n${"=".repeat(60)}`);
    console.log(
      `[ArticleRetryQueue] Starting GUARANTEED processing for ${articles.length} articles`,
    );
    ...
    pendingArticles = pendingArticles.filter(
      (a) => !processedPmids.has(a.pmid),
    );

    console.log(
      `[ArticleRetryQueue] Phase 1 complete: ${initialResult.created.length} studies created, ${pendingArticles.length} remaining`,
    );
    ...
    while (
      ...
      retryAttempt < this.config.maxImmediateRetries
    ) {
      retryAttempt++;
      ...
      const delayMs =
        this.config.immediateRetryDelayMs * Math.pow(1.5, retryAttempt - 1);
      ...
      const retryResult = await this.processArticleBatch(
        ...
        {
          ...options,
          batchSize: this.config.retryBatchSize,
      ...
      pendingArticles = pendingArticles.filter(
        (a) => !retryProcessedPmids.has(a.pmid),
      );

      console.log(
        `[ArticleRetryQueue] Retry ${retryAttempt} result: ${retryResult.created.length} more studies, ${pendingArticles.length} still pending`,
      );

      // If we successfully processed all, exit early
      if (pendingArticles.length === 0) {
        console.log(
          `[ArticleRetryQueue] 🎉 All articles successfully processed!`,
        );
        break;
      }
    }
    ...
    if (pendingArticles.length > 0) {
      ...
      await this.queueForBackgroundRetry(
        jobId,
        pendingArticles,
        searchParams,
        organizationId,
    ...
    results.successRate = (
      (results.studiesCreated /
        (results.totalArticles - results.duplicatesSkipped)) *
      100
    ).toFixed(2);
    ...
    console.log(
      `[ArticleRetryQueue] Studies created: ${results.studiesCreated}`,
    );
    console.log(
      `[ArticleRetryQueue] Duplicates skipped: ${results.duplicatesSkipped}`,
    );
    console.log(
      `[ArticleRetryQueue] Queued for background: ${results.failedArticles.length}`,
    );
  ...
  async processArticleBatch(
    ...
    try {
      ...
      console.log(
        `[ArticleRetryQueue] Getting AI inference for ${articles.length} articles...`,
      );
      ...
      console.log(
        `[ArticleRetryQueue] AI inference completed: ${aiResponse.results.length} results`,
      );

      // Notify caller that AI phase is done, entering study creation phase
      if (typeof options.onAiComplete === "function") {
        await options.onAiComplete(aiResponse.results.length);
      }

      // Get Workflow Config for Auto-QC
      let workflowConfig = { aoiAutoQCPercent: 20, noCaseAutoQCPercent: 20 };
      try {
        const fullConfig = await adminConfigService.getConfig(organizationId);
        if (
          fullConfig &&
          (fullConfig.workflow || fullConfig.configData?.workflow)
        ) {
          workflowConfig =
            fullConfig.workflow || fullConfig.configData.workflow;
        }
        ...
        console.warn(
          `[ArticleRetryQueue] Failed to load workflow config, using defaults`,
          err.message,
        );
      }
      ...
      for (const aiResult of aiResponse.results) {
        try {
          if (!aiResult || !aiResult.pmid || !aiResult.aiInference) {
            console.warn(
              `[ArticleRetryQueue] Skipping invalid AI result: no pmid or aiInference`,
            );
            continue;
          }
          ...
          const duplicateQuery =
            "SELECT c.id FROM c WHERE c.pmid = @pmid AND c.organizationId = @organizationId";
          ...
          if (existingStudies && existingStudies.length > 0) {
            console.log(
              `[ArticleRetryQueue] Skipping duplicate PMID: ${aiResult.pmid}`,
            );
            result.duplicates++;
          ...
          // Set Status & Workflow Track based on Classification + Auto-QC Logic
          // STRICT RULE: We rely ONLY on icsrClassification tag
          ...
          if (
            ...
            study.userTag = "ICSR";
            study.status = "Under Triage Review"; // To ICSR Triage Allocation
            study.subStatus = "triage";
          }
          // 2. AOI Track (Probable AOI)
          else if (
            classification === "Probable AOI" ||
            classification === "AOI"
          ) {
            ...
            // Check Auto-QC Split
            const passAutoQC =
              Math.random() * 100 < (workflowConfig.aoiAutoQCPercent || 20);

            if (passAutoQC) {
              // Subset A: Auto-Pass to Assessment
              study.status = "Ready for Assessment Allocation";
              study.isAutoPassed = true;
              study.subStatus = "assessment";
            } else {
              // Subset B: Remainder to Manual QC (Triage)
              study.status = "qc_triage"; // Correct status for AOI QC Allocation
          ...
          // 3. No Case Track
            ...
            // Check Auto-QC Split
            const passAutoQC =
              Math.random() * 100 < (workflowConfig.noCaseAutoQCPercent || 20);
            ...
            if (passAutoQC) {
              ...
              // Subset B: Remainder to Manual QC (Triage)
              study.status = "qc_triage"; // Correct status for No Case QC Allocation
              study.qaApprovalStatus = "pending";
              study.isAutoPassed = false;
              study.subStatus = "triage";
            }
          ...
          // Save to database
          const savedStudy = await cosmosService.createItem(
            "studies",
            study.toJSON(),
          );
          result.created.push(savedStudy);

          // Per-study progress callback
          if (typeof options.onStudyCreated === "function") {
            await options.onStudyCreated({
              pmid: aiResult.pmid,
              track: study.workflowTrack,
              created: result.created.length,
              total: aiResponse.results.length,
              duplicates: result.duplicates,
            });
          }

          console.log(
            `[ArticleRetryQueue] ✅ Created study for PMID: ${aiResult.pmid}`,
          );
        } catch (studyError) {
          console.error(
            `[ArticleRetryQueue] ❌ Failed to create study for PMID ${aiResult.pmid}:`,
            studyError.message,
          );
          // Find the original article and add to failed
          const originalArticle = articles.find(
            (a) => a.pmid === aiResult.pmid,
          );
          if (originalArticle) {
      ...
      console.error(
        `[ArticleRetryQueue] Batch processing error:`,
        error.message,
      );
      result.failed = articles;
    }
  ...
  async queueForBackgroundRetry(
    ...
    try {
      ...
      console.log(
        `[ArticleRetryQueue] Persisted ${articles.length} articles to retry queue in database`,
      );
    } catch (dbError) {
      console.error(
        `[ArticleRetryQueue] Failed to persist to database:`,
        dbError.message,
      );
    ...
    console.log(
      `[ArticleRetryQueue] Queued ${articles.length} articles for background retry (Job: ${jobId})`,
    );
  }
  ...
  async processBackgroundRetries() {
    ...
    console.log(
      `[ArticleRetryQueue] Processing ${this.retryQueue.size} background retry jobs...`,
    );
    ...
    for (const [jobId, queueItem] of this.retryQueue.entries()) {
      try {
        ...
        if (queueItem.retryCount >= this.config.maxBackgroundRetries) {
          console.log(
            `[ArticleRetryQueue] Job ${jobId} exceeded max retries, moving to failed`,
          );
        ...
        const hoursSinceCreation =
          (Date.now() - new Date(queueItem.createdAt).getTime()) /
          (1000 * 60 * 60);
        if (hoursSinceCreation >= this.config.giveUpAfterHours) {
          console.log(
            `[ArticleRetryQueue] Job ${jobId} exceeded ${this.config.giveUpAfterHours} hours, moving to failed`,
          );
          await this.markJobFailed(jobId, queueItem);
          continue;
        }
        ...
        console.log(
          `[ArticleRetryQueue] Background retry ${queueItem.retryCount} for job ${jobId} (${queueItem.articles.length} articles)`,
        );
        ...
        queueItem.articles = queueItem.articles.filter(
          (a) => !processedPmids.has(a.pmid),
        );
        ...
        if (queueItem.articles.length === 0) {
          console.log(
            `[ArticleRetryQueue] 🎉 Job ${jobId} completed successfully!`,
          );
          await this.markJobComplete(jobId);
        } else {
          queueItem.status = "pending";
          console.log(
            `[ArticleRetryQueue] Job ${jobId}: ${retryResult.created.length} more studies created, ${queueItem.articles.length} still pending`,
          );
        }
      } catch (error) {
        console.error(
          `[ArticleRetryQueue] Error processing job ${jobId}:`,
          error.message,
        );
  ...
  async markJobFailed(jobId, queueItem) {
    ...
    console.warn(
      `[ArticleRetryQueue] ⚠️ Job ${jobId} failed permanently with ${queueItem.articles.length} articles`,
    );
    console.warn(
      `[ArticleRetryQueue] Failed PMIDs: ${queueItem.articles.map((a) => a.pmid).join(", ")}`,
    );
  }
  ...
  async retryAllFailed(organizationId = null) {
    try {
      ...
      const failedJobs = await cosmosService.queryItems(
        "articleRetryQueue",
        query,
        params,
      );
      ...
      console.log(
        `[ArticleRetryQueue] Retrying ${failedJobs.length} failed jobs...`,
      );
