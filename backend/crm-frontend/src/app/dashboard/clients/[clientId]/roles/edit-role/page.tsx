...
export default function ClientEditRolePage() {
    ...
    useEffect(() => {
        if (!roleId || !clientId) {
            setError('Role ID and Client ID are required');
            setLoading(false);
            return;
        }
    ...
    const fetchRoleAndPermissions = async () => {
        try {
            const [roleResponse, permissionStructureResponse] = await Promise.all([
                roleService.getRoleForOrganization(clientId, roleId!),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await roleService.updateRoleForOrganization(clientId, roleId, {
                displayName: formData.displayName,
                description: formData.description,
                permissions: permissionsToSave
            });

            router.push(`/dashboard/clients/${clientId}/roles`);
        } catch (err: any) {
        ...
        return (
            <div className="py-8 px-2 sm:px-6">
                <div className="text-center py-12">
                    ...
                    <button
                        onClick={() => router.push(`/dashboard/clients/${clientId}/roles`)}
