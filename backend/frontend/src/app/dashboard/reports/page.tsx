...
export default function ReportsPage() {
  ...
  const fetchStudies = async () => {
    try {
      ...
      const response = await fetch(
        `${API_CONFIG.BASE_URL}/studies?limit=1000${queryParams}`,
        {
          headers: {
  ...
  const getStudyStatusLabel = (study: Study) => {
    const status = study.status || "Pending Review";
    const track = study.workflowTrack || study.userTag || "";
    ...
    // 2. Medical Review
    if (status === "medical_review" || status === "Medical Review")
      return "Medical Review";

    // 3. QC Data Entry
    if (status === "qc_data_entry" || status === "QC Data Entry")
      return "QC Data Entry";

    // 4. Data Entry
    if (status === "data_entry" || status === "Data Entry") return "Data Entry";

    // 5. Assessment / Allocation
    if (
      status.includes("assessment") ||
      status.includes("Assessment") ||
      status === "qc_allocation" ||
      status === "qc_triage" ||
      status === "under_review"
    ) {
      if (track === "ICSR" || track === "ICSR Track") return "ICSR Assessment";
      if (track === "AOI" || track === "AOI Track") return "AOI Assessment";
      if (track === "No Case" || track === "NoCase" || track === "No Case")
        return "No Case Assessment";
      // If track is unknown but status is assessment
      if (study.userTag === "ICSR") return "ICSR Assessment";
      if (study.userTag === "AOI") return "AOI Assessment";
      if (study.userTag === "No Case") return "No Case Assessment";
      return "Assessment"; // Fallback
    }

    // 6. Triage
    // 'Pending Review', 'triage', 'icsr_triage'
    if (
      ...
      if (track === "ICSR" || track === "ICSR Track") return "ICSR Triage";
      if (track === "AOI" || track === "AOI Track") return "AOI Triage";
      if (track === "No Case" || track === "NoCase" || track === "No Case")
        return "No Case Triage";

      // Fallback based on userTag if track is missing (old data?)
      if (study.userTag === "ICSR") return "ICSR Triage";
      if (study.userTag === "AOI") return "AOI Triage";
      if (study.userTag === "No Case") return "No Case Triage";

      return "Triage"; // Generic Triage
    }
                      ...
                      <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm">
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded-full ${
                            getStudyStatusLabel(study).includes("Triage")
                              ? "bg-blue-100 text-blue-800"
                              : getStudyStatusLabel(study).includes(
                                    "Assessment",
                                  )
