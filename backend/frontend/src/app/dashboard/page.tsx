...
interface Stats {
  ...
  workflowStats?: {
    ...
    completed: number;
    icsrTriage?: number;
    icsrAssessment?: number;
    aoiTriage?: number;
    aoiAssessment?: number;
    noCaseTriage?: number;
    noCaseAssessment?: number;
...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const queryParams = selectedOrganizationId
        ? `?organizationId=${selectedOrganizationId}${dateQuery}`
        : `?${dateQuery.substring(1)}`;
      const queryParamsAmp = selectedOrganizationId
        ? `&organizationId=${selectedOrganizationId}`
        : "";

      // Fetch study statistics
      const statsResponse = await fetch(
        `${API_CONFIG.BASE_URL}/studies/stats/summary${queryParams}`,
        { headers },
      );
      if (statsResponse.ok) {
        ...
        const drugsResponse = await fetch(
          `${API_CONFIG.BASE_URL}/drugs?limit=1000${queryParamsAmp}`,
          { headers },
        );
        ...
        if (drugsResponse.ok) {
          ...
          activeDrugsCount =
            drugsData.drugs?.filter((d: any) => d.status === "active").length ||
      ...
      const auditResponse = await fetch(
        `${API_CONFIG.BASE_URL}/audit?limit=5&sortOrder=desc${queryParamsAmp}`,
        { headers },
      );
      if (auditResponse.ok) {
  ...
  const statsArray = [
    ...
    },
    {
      ...
      value: stats.counts?.medicalReviewers || 0,
      icon: <EyeIcon className="w-8 h-8 text-purple-500" />,
    },
    {
      label: "Total Drugs",
      value: stats.counts?.drugs || 0,
      icon: <BeakerIcon className="w-8 h-8 text-green-500" />,
    },
    {
      label: "QA Reviewed Cases",
      value: stats.counts?.qaReviewed || 0,
  ...
  const workflowBoxes = [
    {
      label: "ICSR Triage",
      value: stats.workflowStats?.icsrTriage || 0,
      icon: DocumentMagnifyingGlassIcon,
      color: "text-blue-600",
      bg: "bg-blue-50",
    },
    {
      label: "ICSR Assessment",
      value: stats.workflowStats?.icsrAssessment || 0,
      icon: ClipboardDocumentListIcon,
      color: "text-blue-700",
      bg: "bg-blue-100",
    },
    {
      label: "AOI Allocation",
      value: stats.workflowStats?.aoiTriage || 0,
      icon: BeakerIcon,
    ...
    {
      label: "AOI QC",
      value: stats.workflowStats?.aoiAssessment || 0,
      icon: ClipboardDocumentListIcon,
      color: "text-purple-700",
      bg: "bg-purple-100",
    },
    {
      label: "No Case Allocation",
      value: stats.workflowStats?.noCaseTriage || 0,
      icon: XCircleIcon,
      color: "text-gray-600",
      bg: "bg-gray-50",
    },
    {
      label: "No Case QC",
      value: stats.workflowStats?.noCaseAssessment || 0,
      icon: ClipboardDocumentListIcon,
      color: "text-gray-700",
      bg: "bg-gray-100",
    },
    {
      label: "Data Entry",
      value: stats.workflowStats?.dataEntry || 0,
    ...
    {
      label: "QC Data Entry",
      value: stats.workflowStats?.qcDataEntry || 0,
    ...
    },
    {
      ...
      value: stats.workflowStats?.medicalReview || 0,
      icon: UserGroupIcon,
      color: "text-pink-600",
      bg: "bg-pink-50",
    },
    {
      ...
      value: stats.workflowStats?.reportsCreated || 0,
      icon: CheckBadgeIcon,
            ...
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-500">
                  {stats.dateStats?.totalCreated === 0
                    ? "No articles created on this date"
                    : ""}
                </span>
              </div>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex flex-col items-center justify-center">
                    ...
                    <p className="text-4xl font-bold text-gray-900 mt-2">
                      {stats.dateStats?.totalReportsCreated || 0}
                    </p>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      <div className="p-3 bg-blue-50 rounded-lg flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-xl font-bold text-blue-800 mt-1">
                          {stats.dateStats?.priorityQueue?.probableAoi || 0}
                        </span>
                      </div>
                      <div className="p-3 bg-purple-50 rounded-lg flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-xl font-bold text-purple-800 mt-1">
                          {stats.dateStats?.priorityQueue?.probableIcsr || 0}
                        </span>
                      </div>
                      <div className="p-3 bg-indigo-50 rounded-lg flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-xl font-bold text-indigo-800 mt-1">
                          {stats.dateStats?.priorityQueue?.probableIcsrAoi || 0}
                        </span>
                      </div>
                      <div className="p-3 bg-orange-50 rounded-lg flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-xl font-bold text-orange-800 mt-1">
                          {stats.dateStats?.priorityQueue?.manualReview || 0}
                        </span>
                      </div>
                      <div className="p-3 bg-gray-50 rounded-lg flex flex-col items-center justify-center text-center">
                        ...
                        <span className="text-xl font-bold text-gray-800 mt-1">
                          {stats.dateStats?.priorityQueue?.noCase || 0}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {workflowBoxes.map((box) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-medium text-gray-500 text-center mb-1 h-8 flex items-center">
                      {box.label}
                    </span>
                    <span className={`text-2xl font-bold ${box.color}`}>
                      {box.value}
                    </span>
                  </div>
                ))}
