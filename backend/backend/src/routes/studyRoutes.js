      ...
      for (const studyData of studiesToProcess) {
        try {
          ...
          if (study.userTag === "ICSR") {
            ...
            // No Case routes to Secondary QC, not directly to reporting
          ...
          if (nextStageId) {
            ...
            // No Case still needs to go through secondary QC — keep as pending
            if (study.userTag === "No Case") {
              study.qaApprovalStatus = "pending";
              study.qaComments = "Auto-routed to Secondary QC via Bulk Process";
            } else {
    ...
    try {
      ...
      if (updates.userTag) {
        ...
        if (study.workflowStage) {
          console.log(
            `[Workflow Debug] Processing update for study ${studyId}`,
          );
          ...
          switch (study.workflowStage) {
            // ---------------------------
            // 1. TRIAGE STAGES
            // Match both naming conventions (TRIAGE_ICSR from trackRoutes allocation, TRIAGE_QUEUE_ICSR legacy)
            // ---------------------------
            case "TRIAGE_ICSR":
            case "TRIAGE_QUEUE_ICSR": // Starting point for most
              if (decision === "ICSR") {
                ...
                // Clear assignment so study appears in AOI assessment pool
                ...
                // Clear assignment so study appears in No Case assessment pool
                study.assignedTo = null;
                study.lockedAt = null;
                study.batchId = null;
              }
            ...
            case "TRIAGE_QUEUE_AOI":
              if (decision === "AOI") {
                ...
                // Clear assignment so study appears in ICSR assessment pool
                study.assignedTo = null;
                study.lockedAt = null;
                study.batchId = null;
              } else if (decision === "No Case") {
          ...
          if (nextStageId) {
            ...
            if (nextClassification) study.classification = nextClassification;

            // Also set userTag and classifiedBy (since updateUserTag won't run)
