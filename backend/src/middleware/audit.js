  ...
  return async (req, res, next) => {
    ...
    res.json = async function (data) {
      try {
        ...
        if (shouldLog) {
          const fullName =
            typeof req.user.getFullName === "function"
          ...
          let location = null;
          try {
            location = await geolocationService.getCountryFromIP(ipAddress);
          } catch (e) {
            // Ignore location lookup failure
          }
        ...
        console.error(
          "Audit logging middleware failed (non-blocking):",
          auditError,
        );
        // Continue to send response even if audit fails
      }
  ...
  try {
    ...
    if (beforeValue !== null && afterValue !== null) {
      ...
      if (changes.length > 0) {
        ...
        enrichedDetails = baseDetails
          ? baseDetails
          : createAuditDescription(action, resource, resourceId, changes);
      }
    }
    ...
    let location = null;
    if (ipAddress) {
      location = await geolocationService
