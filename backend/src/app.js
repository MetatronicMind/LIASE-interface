...
if (cacheService && typeof cacheService.connect === "function") {
  ...
  console.warn("Cache service not properly loaded or connect method missing");
}
...
const studyRoutes = require("./routes/studiesRoutes");
...
console.log(
  "drugRoutes methods:",
  drugRoutes.stack ? drugRoutes.stack.length : "no stack",
);
...
app.get("/api/health", async (req, res) => {
  try {
    ...
    try {
      ...
      await cosmosService.queryItems(
        "organizations",
...
async function startServer() {
  ...
  console.log(
    `- COSMOS_DB_DATABASE_ID: ${process.env.COSMOS_DB_DATABASE_ID || "NOT SET"}`,
  );
  ...
  try {
    ...
    console.log(
      `🔍 Queue status: http://localhost:${PORT}/api/drugs/queue-status`,
    );
...
app.get("/api/health", (req, res) => {
  ...
  if (memoryUsagePercent > 90) {
    ...
    (healthMetrics.requestCount > 100 &&
      healthMetrics.errorCount / healthMetrics.requestCount > 0.1)
  ) {
...
setInterval(() => {
  ...
  if (memoryUsagePercent > 90) {
    console.error(
      `⚠️  CRITICAL: Memory usage at ${Math.round(memoryUsagePercent)}% (${Math.round(memUsage.heapUsed / 1024 / 1024)}MB)`,
    );
    ...
    console.warn(
      `⚠️  WARNING: Memory usage at ${Math.round(memoryUsagePercent)}% (${Math.round(memUsage.heapUsed / 1024 / 1024)}MB)`,
    );
  }
