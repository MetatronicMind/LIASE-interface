...
class TrackAllocationService {
    ...
    async getTrackAllocationPercentage(organizationId, trackType) {
        try {
            ...
            console.warn(`Failed to fetch allocation percentage for ${trackType}:`, err);
        }
    ...
    async moveToAllocation(organizationId, trackType, user, count = null) {
        let studies = await this.getStudiesInTrack(organizationId, trackType, 'triage');

        // If count is specified, limit the number of studies to process
        if (count !== null && count !== undefined && count > 0 && count < studies.length) {
        ...
        const userName = user.getFullName ? user.getFullName() : `${user.firstName} ${user.lastName}`;
    ...
    async allocateTrack(organizationId, trackType, retainPercentage, user) {
        ...
        const studies = await this.getStudiesInTrack(organizationId, trackType, 'allocation');
    ...
    async routeStudy(organizationId, studyId, destination, user) {
        const studyData = await cosmosService.getItem('studies', studyId, organizationId);
