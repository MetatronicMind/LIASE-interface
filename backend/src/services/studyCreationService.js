...
class StudyCreationService {
  ...
  async startStudyCreationJob(jobId, pmids, params, userId, organizationId) {
    ...
    try {
      ...
      if (
        ...
        workflowConfig.configData.stages
      ) {
      ...
      console.warn(
        "Failed to fetch workflow config, using default status:",
        error,
      );
    }

    // Start processing asynchronously - don't await
    this.processStudiesAsync(
      jobId,
      pmids,
      params,
      userId,
  ...
  async processStudiesAsync(
    ...
    try {
            ...
            notificationService.notifyProgress(
              `Processing studies: ${jobData.processed}/${jobData.total}`,
              {
                ...
                percentage: Math.round(
                  (jobData.processed / jobData.total) * 100,
                ),
              },
              {
                jobId,
      ...
      console.log(
        `Study creation job ${jobId} completed: ${created.length} created, ${skipped.length} skipped, ${errors.length} errors`,
      );
  ...
  async processSingleStudy(
    ...
    try {
      ...
      console.error(
        `Error getting AI inference for PMID ${article.pmid}:`,
        error,
      );
    ...
    if (aiInferenceData) {
      ...
      studyData.keyEvents = aiInferenceData.Key_events
        ? aiInferenceData.Key_events.split(",").map((e) => e.trim())
        : [];
      studyData.relevantDates = aiInferenceData.Relevant_dates;
      studyData.administeredDrugs = aiInferenceData.Administered_drugs
        ? aiInferenceData.Administered_drugs.split(",").map((d) => d.trim())
        : [];
      ...
      studyData.identifiableHumanSubject =
        aiInferenceData.Identifiable_human_subject === "Yes";
      studyData.textType = aiInferenceData.Text_type;
      studyData.authorPerspective = aiInferenceData.Author_perspective;
      studyData.confirmedPotentialICSR =
        aiInferenceData.Confirmed_potential_ICSR === "Yes";
    ...
    console.log(
      `Study created successfully for PMID: ${article.pmid} with ${aiInferenceData ? "AI inference data" : "basic data only"}`,
    );
  }
  ...
  async routeStudyBasedOnClassification(studyData, organizationId) {
    ...
    try {
      const allocConfigReq = await adminConfigService.getConfig(
        organizationId,
        "allocation",
      );
      allocationConfig = allocConfigReq?.configData || {};

      const workflowConfigReq = await adminConfigService.getConfig(
        organizationId,
        "workflow",
      );
      workflowConfig = workflowConfigReq?.configData || {};
    } catch (error) {
      console.warn("Failed to fetch configs, using defaults:", error);
    }

    // Prioritize workflow config (from settings page) over allocation config
    const aoiAllocationPercentage =
      workflowConfig?.aoiAllocationPercentage ??
      (allocationConfig?.aoiAllocationPercentage || 10);
    const noCaseAllocationPercentage =
   ...
   */
  cleanupOldJobs() {
    if (this.jobResults.size > 200) {
      // Keep at most 200 total jobs
