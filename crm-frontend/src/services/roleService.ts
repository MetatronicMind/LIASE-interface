...
class RoleService {
  ...
  // =================================================================
  // ORGANIZATION-SPECIFIC ROLE MANAGEMENT (Super Admin for CRM Clients)
  // =================================================================
  ...
  async getRolesByOrganization(orgId: string): Promise<Role[]> {
    ...
    if (!response.ok) {
      await this.handleAuthError(response);
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Failed to fetch organization roles');
    }
  ...
  async getRoleForOrganization(orgId: string, roleId: string): Promise<Role> {
    ...
    if (!response.ok) {
      await this.handleAuthError(response);
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Failed to fetch organization role');
    }
  ...
  async createRoleForOrganization(orgId: string, roleData: Partial<Role>): Promise<Role> {
    ...
    if (!response.ok) {
      await this.handleAuthError(response);
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Failed to create organization role');
    }
  ...
  async updateRoleForOrganization(orgId: string, roleId: string, roleData: Partial<Role>): Promise<Role> {
    ...
    if (!response.ok) {
      await this.handleAuthError(response);
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Failed to update organization role');
    }
  ...
  async deleteRoleForOrganization(orgId: string, roleId: string): Promise<void> {
    ...
    if (!response.ok) {
      await this.handleAuthError(response);
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || 'Failed to delete organization role');
    }
  }
}
