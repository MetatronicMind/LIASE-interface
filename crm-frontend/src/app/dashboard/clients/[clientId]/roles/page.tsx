...
export default function ClientRolesPage() {
    ...
    const fetchRoles = async () => {
        try {
            const data = await roleService.getRolesByOrganization(clientId);
            setRoles(data);
            setError(null);
        } catch (err: any) {
    ...
    const handleCreateRole = async () => {
        ...
        try {
            await roleService.createRoleForOrganization(clientId, {
                name: newRole.name,
    ...
    const handleDeleteRole = async (roleId: string, roleName: string) => {
        ...
        try {
            await roleService.deleteRoleForOrganization(clientId, roleId);
            await fetchRoles();
            setError(null);
        } catch (err: any) {
        ...
        <div className="py-8 px-2 sm:px-6">
            <div className="max-w-6xl mx-auto">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900 mb-1">Client Roles</h2>
                        <div className="text-gray-600 text-sm">Manage roles and permissions for this client</div>
                    </div>
                                    ...
                                    <tr key={role.id} className="border-b border-gray-100 hover:bg-gray-50">
                                        ...
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className={`px-2 py-1 rounded text-xs font-semibold ${role.isSystemRole
                                        ...
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-2">
                                                <button
                                                    onClick={() => router.push(`/dashboard/clients/${clientId}/roles/edit-role?id=${role.id}`)}
