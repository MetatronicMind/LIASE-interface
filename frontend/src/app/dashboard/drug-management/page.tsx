        ...
        <div className="bg-white rounded-xl shadow border border-blue-100 p-0 overflow-x-auto">
            ...
            <table className="min-w-full text-sm border-separate border-spacing-0">
              <thead>
                <tr className="bg-blue-50 text-blue-900">
                  <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">Drug Name</th>
                  <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">Manufacturer</th>
                  <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">RSI</th>
                  <th className="py-3 px-4 text-left font-semibold whitespace-nowrap">Next Search</th>
              ...
              <tbody>
                {pagedDrugs.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="text-center py-6 text-gray-400">No drugs found.</td>
                  </tr>
                ) : (
                  pagedDrugs.map((drug, idx) => (
                    <tr key={drug.name + idx} className="border-b last:border-b-0 border-blue-50">
                      <td className="py-3 px-4 font-bold text-blue-900 align-top">
                        {drug.name}
                        <div className="text-xs font-mono text-gray-700 bg-gray-50 rounded px-1 py-0.5 mt-1 w-fit">{drug.query}</div>
                      </td>
                      <td className="py-3 px-4 align-top text-blue-900">{drug.manufacturer}</td>
                      <td className="py-3 px-4 align-top">
                        <span className="bg-blue-100 text-blue-700 font-mono font-semibold px-2 py-1 rounded text-xs">{drug.rsi}</span>
                      </td>
                      <td className="py-3 px-4 align-top text-blue-900">{drug.nextSearch ? new Date(drug.nextSearch).toLocaleDateString() : ""}</td>
                      <td className="py-3 px-4 align-top">
                      ...
                      <td className="py-3 px-4 align-top">
                        <div className="flex gap-2">
                          ...
                          <button className="bg-red-100 hover:bg-red-200 text-red-600 p-2 rounded transition" title="Delete" onClick={() => handleDelete(drugs.indexOf(drug))}>
          ...
          <div className="sm:hidden divide-y divide-blue-100">
            {pagedDrugs.length === 0 ? (
              ...
              pagedDrugs.map((drug, idx) => (
                <details key={drug.name + idx} className="py-2 px-2 group">
                  <summary className="flex justify-between items-center cursor-pointer font-bold text-blue-900 text-base">
                    <div className="flex flex-col flex-1">
                      <span>{drug.name}</span>
                      <span className="text-xs font-normal text-blue-700">{drug.manufacturer}</span>
                      <span className="text-xs font-mono text-indigo-700">{drug.rsi}</span>
                    </div>
                    <span className="ml-2 bg-green-100 text-green-700 font-semibold px-2 py-0.5 rounded-full text-xs">{drug.status}</span>
                  ...
                  <div className="mt-2 text-sm text-blue-900">
                    <div className="mb-1"><span className="font-semibold text-indigo-700">Query:</span> <span className="font-mono">{drug.query}</span></div>
                    <div className="mb-1"><span className="font-semibold text-indigo-700">Next Search:</span> {drug.nextSearch ? new Date(drug.nextSearch).toLocaleDateString() : ""}</div>
            ...
            <div className="bg-white rounded-xl shadow-2xl border border-blue-100 w-full max-w-lg relative animate-fade-in flex flex-col max-h-[90vh]">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-y-auto px-6 py-4 space-y-4">
                <div>
                  ...
                  <input name="name" value={form.name} onChange={handleChange} className={`w-full border-2 rounded-lg px-4 py-2 text-base focus:outline-none transition ${errors.name ? 'border-red-500' : 'border-blue-600 focus:border-blue-800'} bg-white text-blue-900 font-semibold`} />
                  {errors.name && <div className="text-red-500 text-sm mt-1">{errors.name}</div>}
                </div>
                <div>
                  <label className="block text-base font-bold text-indigo-700 mb-2">Query</label>
                  <input name="query" value={form.query} onChange={handleChange} className={`w-full border-2 rounded-lg px-4 py-2 text-base font-mono focus:outline-none transition ${errors.query ? 'border-red-500' : 'border-blue-600 focus:border-blue-800'} bg-white text-blue-900`} />
                ...
                <div>
                  ...
                  <input name="manufacturer" value={form.manufacturer} onChange={handleChange} className={`w-full border-2 rounded-lg px-4 py-2 text-base focus:outline-none transition ${errors.manufacturer ? 'border-red-500' : 'border-blue-600 focus:border-blue-800'} bg-white text-blue-900`} />
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    ...
                    <input name="rsi" value={form.rsi} onChange={handleChange} className={`w-full border-2 rounded-lg px-4 py-2 text-base font-mono focus:outline-none transition ${errors.rsi ? 'border-red-500' : 'border-blue-600 focus:border-blue-800'} bg-white text-blue-900`} />
                ...
                <div>
                  ...
                  <select name="status" value={form.status} onChange={handleChange} className="w-full border-2 rounded-lg px-4 py-2 text-base focus:outline-none border-blue-600 focus:border-blue-800 bg-white text-blue-900">
