...
export default function TrackTriagePage({ trackType, trackDisplayName }: TrackTriagePageProps) {
    ...
    const handleStartTriage = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE}/track/${trackType}/allocate-batch`, {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                ...
                if (cases.length === 0) {
                    toast.error(`No ${trackDisplayName} cases available for triage at this time.`, {
                        duration: 4000,
                        icon: '📭'
                    });
                    dispatch(setSidebarLocked(false));
                    ...
                    toast.success(`Allocated ${cases.length} case(s) for ${trackDisplayName} triage`, {
                        duration: 3000
                    });
                }
            } else {
    ...
    const getFinalClassification = (study: Study): string | null => {
        ...
        if (isProbableICSR) {
            return isProbableAOI ? "Probable ICSR/AOI" : "Probable ICSR";
        }

        if (icsrLower === "no case") {
            return isProbableAOI ? "Probable AOI" : "No Case";
        }
    ...
    const getClassificationColor = (classification?: string) => {
        switch (classification) {
            ...
            case "Manual Review": return "bg-indigo-100 text-indigo-800 border-indigo-200";
        ...
        return (
            <div className="flex h-full items-center justify-center bg-gray-50 min-h-screen">
                <div className="text-center p-8 bg-white rounded-lg shadow-md border border-gray-200 max-w-md">
                    ...
                    <p className="text-gray-600">You do not have permission to view the {trackDisplayName} Triage section.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="h-full flex flex-col bg-gray-50 min-h-screen">
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm z-10">
                <div className="flex items-center space-x-4">
                    <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{trackDisplayName} Triage Assessment</h1>
                ...
                {allocatedCases.length === 0 ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center">
                        <div className="text-center max-w-2xl mx-auto p-8 bg-white rounded-2xl shadow-sm border border-gray-200">
                            ...
                            <h2 className="text-3xl font-bold text-gray-900 mb-4">Start {trackDisplayName} Triage</h2>
                        ...
                        <div className="flex flex-col lg:flex-row gap-6 h-full">
                            ...
                            <div className="w-full lg:w-1/2 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
                                <TriageStudyDetails
                                    ...
                                    onUpdate={(updated: any) => setAllocatedCases(prev => prev.map(s => s.id === updated.id ? { ...s, ...updated } : s))}
