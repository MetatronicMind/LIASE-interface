...
export default function ClientSelector() {
  ...
  useEffect(() => {
    ...
    const isSuperAdmin = user?.role === 'superadmin' || (user as any)?.isSystemRole;
  ...
  const fetchOrganizations = async () => {
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/organizations`, {
        headers: {
  ...
  return (
    <div className="flex items-center space-x-2 bg-white p-2 rounded shadow-sm border border-gray-200 ml-4">
      ...
      <select
        ...
        onChange={(e) => dispatch(setSelectedOrganizationId(e.target.value || null))}
        disabled={loading}
      >
